...
def generate_post_transformation(): 
    ...
    media_asp_volume_step_1 = 180
    transf_plate_mix_volume_step_1 = 50 #?
    sel_plate_mix_volume_step_1 = 100 #?
    num_mix_step_1 = 3

    # selection plate to selection plate variables
    sel_to_sel_asp_volume = 1 # check that this can even be done...
    # keep the other variables the same as step 1

    # step 4 variables (selection plate #3 -> master/freezer plate)
    sel_to_master_asp_volume = 100
    ...
    incubation_time_between_readings = [0,1,0,0] # 1 hour

    # tear down variables
    selection_incubation_time = [0,3,0,0]  # --> 0 days, 3 hours, 0 minutes, 0 seconds


    #* SOFTLINX .slvp PROTOCOL - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    softLinx = SoftLinx("Post Transformation", "C:\\Users\\svcaibio\\Desktop\\Debug\\post_transformation.slvp")
    ...
    plate_id= 0  # keep track of current plate id
    ...
    plate_id += 1
    tear_down(current_softLinx=softLinx, incubator_plate_id=plate_id, incubation_time=selection_incubation_time, shaker_speed=30)
    ...
    for i in range(2): 
        ...
        plate_id += 1
        tear_down(current_softLinx=softLinx, incubator_plate_id=plate_id, incubation_time=[0,3,0,0], shaker_speed=30)
    ...
    # clear SOLO deck and move new plate to incubator
    plate_id += 1
    tear_down(current_softLinx=softLinx, incubator_plate_id=plate_id,incubation_time=[0,8,0,0])
...
# ---------------------------------------------------------------------------------
def tear_down(current_softLinx:SoftLinx, incubator_plate_id, incubation_time, shaker_speed): 
    """ tear_down
