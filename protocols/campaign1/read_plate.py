...
def read_OD590(plate_id, directory_path):
    ...
    # SoftLinx Protocol--------------------------------------------------------------
    softLinx = SoftLinx(
        f"Absorbance {plate_id}",
        os.path.join(directory_path, f"Absorbance_{plate_id}.slvp"),
    )
    ...
    softLinx.plateCraneMovePlate(
        ["SoftLinx.Solo.Position4"], ["SoftLinx.Solo.Position4"]
    )
    ...
    # replace lid
    softLinx.plateCraneReplaceLid(
        ["SoftLinx.PlateCrane.LidNest2"], ["SoftLinx.PlateCrane.LidNest1"]
    )
