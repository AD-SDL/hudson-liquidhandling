...
if most_recent_txt_path: 
    ...
    if os.path.exists(run_log_path):
        ...
        with open(run_log_path, 'r') as run_log: 
            for log_line in run_log:
                ...
                if (str(log_line_filename) == str(most_recent_txt_path)) and (str(log_line_timestamp) == str(most_recent_txt_timestamp)):
                    present_in_log = True
    ...
    if present_in_log == False:  # only if protocol is not present in log (not already run)
        ...
        with open(most_recent_txt_path) as f:
            ...
            required_files_list = [each.strip() for each in required_files_list]  # remove newline character

            file_present = ""  # store status of each file in instructions folder (1=present, 0=absent)
            for i in range(len(required_files_list)):
                file_path_to_check = f.name.replace(os.path.basename(f.name), required_files_list[i])
                if os.path.exists(file_path_to_check):
                    ...
                    file_present += "0"
                    print("A required file is missing: " + os.path.basename(f.name))  # will print to log file
