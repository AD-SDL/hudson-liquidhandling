...
# * Program Variables
default_z_shift = 2
reservoir_z_shift = 0.5  # z shift for 12 channel reservoirs
...
muconate_dilution_volumes = [
    ...
    30,
    24,
]  # last column excluded (can't aspirate 0uL)
...
for i in range(1, 13):
    soloSoft.aspirate(
        position="Position3",
        aspirate_volumes=Reservoir_12col_Agilent_201256_100_BATSgroup().setColumn(
            buffer_12_channel_column, buffer_dilution_volumes[i - 1]
        ),
...
for i in range(1, 12):
    soloSoft.aspirate(
        position="Position3",
        aspirate_volumes=Reservoir_12col_Agilent_201256_100_BATSgroup().setColumn(
            muconate_12_channel_column, muconate_dilution_volumes[i - 1]
        ),
        aspirate_shift=[0, 0, reservoir_z_shift],
        pre_aspirate=buffer_blowoff,
    )
    soloSoft.dispense(
        position="Position5",
        dispense_volumes=Plate_96_PlateOne_1833_9600_ConicalBottomStorage().setColumn(
            i, muconate_dilution_volumes[i - 1]
        ),
        dispense_shift=[0, 0, default_z_shift],
        blowoff=buffer_blowoff,
        mix_at_finish=True,
