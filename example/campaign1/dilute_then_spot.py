"""
#TODO ask becca what she wants the concentration to be in the first well -> pure stock or 10 fold dilution
...
for i in range(1,3):  # loop once for each half of the plate
    ...
    # Dilute into first column of serial dilution plate
    soloSoft.getTip("Position7") # 50uL tips 
    soloSoft.aspirate(
        position="Position5", 
        aspirate_volumes=Plate_96_Corning_3635_ClearUVAssay().setColumn(stock_start_column, dilution_transfer_volume),
    ...
    soloSoft.dispense(
        position="Position6", 
        dispense_volumes=Plate_96_Corning_3635_ClearUVAssay().setColumn((6*(i-1))+1, dilution_transfer_volume),
        dispense_shift=[0,0,default_z_shift], 
        mix_at_finish=True,
