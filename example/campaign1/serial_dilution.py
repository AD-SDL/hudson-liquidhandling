...
blowoff_volume = 20
stock_mix_volume = (
    50  # TODO: Calculate these mix volumes based on total volume after calculations
)
...
if dilution_method_number == 1:
    dilution_factor = (lowest_concentration / highest_concentration) ** (
        1 / num_columns
    )  # dilution factor
    serial_transfer_volume = (
        dilution_factor * desired_final_volume
    )  # TODO: does this need to be an int? or float? ect.
...
# * Method 2
    ...
    serial_transfer_volume = (
        dilution_factor * desired_final_volume
    )  # TODO: do these need to be an int? or float? ect.
    diluent_transfer_volume = desired_final_volume - serial_transfer_volume
...
# * Method 3
    ...
    for i in range(len(dilution_concentrations_list)):
        if i == 0:
            current_dilution_factor = (
                dilution_concentrations_list[i] / highest_concentration
            )
        else:
            current_dilution_factor = (
                dilution_concentrations_list[i] / dilution_concentrations_list[i - 1]
            )
...
if dilution_method_number == 1 or dilution_method_number == 2:
    ...
    soloSoft.aspirate(
        ...
        aspirate_volumes=GenericPlate96Well().setColumn(
            1, serial_transfer_volume
        ),  # TODO make sure the user places stock solution in this location
    ...
    for i in range(1, num_columns + 1):
        soloSoft.aspirate(
            position="Position5",
            aspirate_volumes=ZAgilentReservoir_1row().setColumn(
                1, diluent_transfer_volumes[i - 1]
            ),
        ...
        soloSoft.dispense(
            position="Position3",
            dispense_volumes=GenericPlate96Well().setColumn(
                i, diluent_transfer_volumes[i - 1]
            ),
            dispense_shift=[0, 0, 2],
        )
    ...
    for i in range(1, num_columns):
        ...
        soloSoft.dispense(
            position="Position4",
            dispense_volumes=GenericPlate96Well().setColumn(
                i + 1, serial_transfer_volumes[i - 1]
            ),
            dispense_shift=[0, 0, 2],
            mix_at_finish=True,
            mix_cycles=num_mixes,
